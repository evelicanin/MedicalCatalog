            ...
            <div class="my-3 my-md-5">
                <div class="container">
	                ...
	                <?php // add_zdravstvena_ustanova(); 
                        if(isset($_POST['add_zdravstvena_ustanova'])) 
                        {
                            ...
                            if(isset($_GET['id'])) 
                            {
                                $spec_id = $_GET['id']; // ID SUB-SPECIJALIZACIJE --> subspec_id
                                
                                // proslijedjujemo RSS jer je SUBspecijalizacija u pitanju
                                $insert_user = query("INSERT INTO ustanove_staz(spec_id, u_id, iznos, oznaka, user_unio) VALUES ('{$spec_id}', '{$ustanova}', '{$iznos}', 'RSS', '{$user_unio}') ");
                                confirm($insert_user);
                    ...
                    <div class="page-header">                
                      <h1 class="page-title">
                        Dodavanje zdravstvenih ustanova u kojima se obavlja subpecijalizacija
                      </h1>                  
                    </div>
                    <div class="btn-list text-right mb-2 mr-3" style="margin-top:-60px;">                  
                        <?php    
                            echo '<a class="btn btn-lg btn-secondary" href="view_rez_subspecijalizacija.php?id='. $_GET['id'] .'">
						    <i class="fe fe-chevron-left"></i> Nazad na subpecijalizaciju
                            </a>';
                        ?>
                    </div> 
                        ...
                        <div class="col-12">
                            <form class="card" action="" method="post">
                                ...
                                <div class="card-footer text-right">
                                  <button type="submit" name="add_zdravstvena_ustanova" class="btn btn-primary"><i class="fe fe-database"></i> Spasi</button>
                                </div>
                            </form>                    
                        </div>
                    </div>
