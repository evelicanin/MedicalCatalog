...
<?php user_is_logged();?> <!-- ako user nije logovan, vrati ga nazad na login -->
	...
	<?php 
		...
		if(isset($_GET['id'])) 
		{
			...
			$query = query("SELECT * FROM drzavljanstva WHERE d_id = " . escape_string($_GET['id']) . " ");
			confirm($query);

			while($row = fetch_array($query))
			{
				$d_id       = escape_string($row['d_id']);
				$d_title    = escape_string($row['d_title']);		 
			}
		}
                    ...
                    <div class="row row-cards row-deck">
                        <div class="col-12">
                            <form class="card" action="" method="post">
                                <div class="card-body">
                                  <h3 class="card-title">Editovanje državljanstva u šifarniku</h3>
                                  <div class="row">
                                    <div class="col-sm-12 col-md-12">
                                      <div class="form-group">
                                        <label class="form-label">Državljanstvo</label>
                                        <input type="text" name="naziv" class="form-control" placeholder="Naziv" value="<?php echo $d_title; ?>">
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                <div class="card-footer text-right">
                                  <button type="submit" name="update_drzavljanstvo" class="btn btn-primary"><i class="fe fe-database"></i> Spasi</button>
                                </div>
                            </form>          

                            <?php update_drzavljanstvo();  ?>
