	...
	<?php 
		if(isset($_GET['id']))
		{
            ...
            $query2 = query("SELECT us_id from ustanove_uplate WHERE upl_id=" . escape_string($_GET['id']) . " ");
            confirm($query2);
            ...
            $query = query("DELETE FROM ustanove_uplate WHERE upl_id = " . escape_string($_GET['id']) . " ");
            confirm($query);
            
            // DELETE gotov, sada idemo nazad na ustanovu kojoj je pripadala izbrisana uplata                               
            set_message("Brisanje uspješno!");	
            redirect('../../../public/ustanove_uplate_sub.php?id='.$go_back_id.''); // idemo nazad           
		}
		else 
		{
