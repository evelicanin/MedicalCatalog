	...
	<?php 
		if(isset($_GET['id']))
		{
            ...
            $query = query("DELETE FROM ustanove_staz WHERE us_id = " . escape_string($_GET['id']) . " ");
            confirm($query);
            ...
            redirect('../../../public/view_rez_subspecijalizacija.php?id='.$go_back_id.''); // idemo nazad
		...
		else 
		{
		    ...
		    while($row = fetch_array($query2)) {
                redirect('../../../public/view_rez_subspecijalizacija.php?id='.htmlentities($row["spec_id"]).''); // idemo nazad
            }
		}
	?>